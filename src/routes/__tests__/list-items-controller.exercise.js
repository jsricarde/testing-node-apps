...
test('setListItem returns a 404 error if the list iten does not exists', async () => {
...
test('createListItem creates and returns a list item', async () => {
  const user = buildUser()
  const book = buildBook()
  const createdListItem = buildListItem({ bookId: book.id, ownerId: user.id })
...
test('createListItem returns a 400 error if the user already has a list item for the given book', async () => {
  ...
  const book = buildBook({ id: fakeBookId })
  const existingListItem = buildListItem({ bookId: book.id, ownerId: user.id })
